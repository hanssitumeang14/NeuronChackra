export default function Information() {
  return (
    <div className="flex items-center rounded-2xl justify-center bg-gradient-to-b from-[#1a002b] via-[#0c0f3a] to-[#00010a] py-10 px-5 mb-16">
      <div id="accordion-collapse" data-accordion="collapse" className="w-full max-w-3xl">
        <h1 className="text-4xl font-cinzel text-center text-cyan-300 mb-10 drop-shadow-[0_0_5px_#00ffff]">
          Penjelasan
        </h1>
        {[
          {
          ...
          return (
            <div
              key={idx}
              className="mb-5 border border-indigo-700 rounded-xl shadow-xl bg-[#232643]/90 backdrop-blur-md"
            >
              <h2 id={headingId}>
                <button
                  type="button"
                  className="flex items-center justify-between w-full p-5 font-semibold text-cyan-200 border-b border-indigo-800 rounded-t-xl focus:outline-none hover:bg-indigo-950/40 transition duration-300 ease-in-out"
                  data-accordion-target={`#${id}`}
                  aria-expanded="false"
                  aria-controls={id}
                >
                  <span className="text-lg text-cyan-300 drop-shadow-[0_0_2px_#00ffff]">
                    {section.title}
                  </span>
                  <svg
                    data-accordion-icon
                    className="w-4 h-4 shrink-0 transform transition-transform ease-in-out duration-200 text-cyan-400"
                    aria-hidden="true"
              ...
              <div id={id} className="hidden" aria-labelledby={headingId}>
                <div className="p-5 bg-[#1e213a] border-t border-indigo-700 rounded-b-xl">
